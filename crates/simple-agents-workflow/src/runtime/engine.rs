...
pub(super) async fn execute_node(
    ...
    match &node.kind {
        ...
        NodeKind::RetryCompensate {
            ...
            if output.is_null() {
                ...
                let compensation = executor
                    ...
                    .await
                    .map_err(
                        |compensation_error| WorkflowRuntimeError::RetryCompensateFailed {
                            node_id: node.id.clone(),
        ...
        NodeKind::HumanInTheLoop {
            ...
            let decision_value = resolve_path(&scoped_input, decision_path).ok_or_else(|| {
        ...
        NodeKind::Router { routes, default } => {
            ...
            enforce_expression_scope_budget(
                &node.id,
                &scoped_input,
                runtime.options.security_limits.max_expression_scope_bytes,
            )?;
            let mut selected = default.clone();
            for route in routes {
                let matched =
                    expressions::evaluate_bool(&route.when, &scoped_input).map_err(|reason| {
                        WorkflowRuntimeError::InvalidRouterExpression {
                            node_id: node.id.clone(),
        ...
        NodeKind::Reduce {
            ...
            operation,
            next,
        } => runtime.execute_reduce_node(step, node, source, operation, next, scope),
        NodeKind::Subgraph { graph, next } => {
            ...
            scope
                .record_node_output(&node.id, subgraph_output.clone(), ScopeCapability::MapWrite)
                .map_err(|source| WorkflowRuntimeError::ScopeAccess {
                    node_id: node.id.clone(),
                    source,
                })?;
        ...
        NodeKind::Batch { items_path, next } => {
            runtime.execute_batch_node(step, node, items_path, next, scope)
        }
        NodeKind::Filter {
            items_path,
            expression,
