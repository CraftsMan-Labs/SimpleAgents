...
This matrix is the contract baseline for Rust, Node, and Python surfaces.

## Capability Matrix

| Capability | Rust core | Node | Python |
|---|---|---|---|
| Prompt completion | Yes | Yes | Yes |
| Message completion | Yes | Yes | Yes |
| Structured tool-call results | Yes | Yes | Yes |
| Healing modes (`healed_json`, `schema`) | Yes | Yes | Yes |
| Streaming events | Yes | Yes | Yes |
| Streaming terminal event (`done`) | Yes | Yes | Yes |
| Canonical finish reasons (`stop`, `length`, `content_filter`, `tool_calls`) | Yes | Yes | Yes |
...
1. `capability-contract-gates` in `.github/workflows/bindings-ci.yml`
   ...
   - Verifies shared fixture contract checks in Node and Python.
...
- `parity-fixtures/binding_contract.json` is the source of truth for:
  ...
  - binding-specific symbol expectations (Node/Python)
