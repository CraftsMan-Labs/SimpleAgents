...
**CRITICAL**: This section defines absolute rules for AI assistants working on this project.

### What NOT to Create (NEVER)

**NEVER create the following files without explicit user instruction:**

1. ❌ **Completion Reports** (`PHASE*_COMPLETION.md`, `*_COMPLETION_REPORT.md`, etc.)
2. ❌ **Summary Files** (`SUMMARY.md`, `SESSION_SUMMARY.md`, etc.)
3. ❌ **Progress Reports** (`PROGRESS.md`, `STATUS.md`, etc.)
4. ❌ **Changelog Files** (`CHANGELOG.md`, `CHANGES.md`, etc.)
5. ❌ **Meeting Notes** (`NOTES.md`, `MEETING_*.md`, etc.)
6. ❌ **Architecture Decision Records** (`ADR_*.md`, `DECISION_*.md`, etc.)
7. ❌ **Planning Documents** (unless explicitly in TODO.md)

### Why This Rule Exists

- **User Control**: The user decides what documentation is needed
- **No Assumptions**: Don't assume documentation patterns from existing files
- **Focus on Code**: Your job is to write code, tests, and API docs - not project management docs
- **Ask First**: If you think a document would be helpful, ASK the user first

### What You SHOULD Create (Without Asking)

✅ **Source Code** (`.rs` files) - This is your primary job
✅ **Tests** (unit, integration, doc tests) - Required for quality
✅ **API Documentation** (doc comments) - Required for public APIs
✅ **Code Examples** (`examples/*.rs`) - Helpful for demonstrating features
✅ **README.md in crates** - If crate doesn't have one and needs basic info

### What You SHOULD Ask About First

🤔 **Before creating:**
- Any `.md` file at project root (except crate READMEs)
- Any tracking/reporting document
- Any architectural documentation beyond code comments
- Any process/workflow documentation
...
1. Complete task
2. Run tests
3. Update TODO.md (if tracking progress)
4. Tell user what was done
5. Let user decide if they want documentation
```
...
Then and ONLY then, create those files.

### Exception: TODO.md Updates

✅ **Updating TODO.md is ALLOWED** (and encouraged) for:
- Checking off completed tasks
- Updating progress percentages
- Updating test counts
- Updating metrics

This is task tracking, not report writing.

### Golden Rule

**"Code first, ask about docs"**

If you're unsure whether to create a file:
...
Use the `metrics` crate for Prometheus-compatible metrics. Metrics should be recorded in hot paths to monitor:
...
Use consistent naming with labels for dimensions:

```rust
// Counter: Total requests
metrics::counter!(
    "simple_agents_requests_total",
    "provider" => "openai",
    "model" => "gpt-4",
    "status" => "success"
).increment(1);
...
### RequestTimer Pattern

Use the `RequestTimer` helper to automatically record metrics:
...
Enable Prometheus exporter with the `prometheus` feature flag:
...
Use the `governor` crate with token bucket algorithm. Support both per-instance and shared (per-API-key) rate limiting.
...
// Shared rate limiting (across instances with same API key)
let config = RateLimitConfig::shared(50, 100);
...
When receiving 429 (rate limit) responses, extract and respect `retry-after` headers:
...
**Use synchronous validation** for:
- CPU-bound checks (string validation, ranges, formats)
- Checks that complete in <1ms
- Constructor validation
- Builder `build()` methods

**Use asynchronous validation** for:
- I/O-bound checks (database lookups, external API calls)
- Checks that may take >1ms
- Validation requiring network access
...
impl ApiKey {
    pub fn new(key: impl Into<String>) -> Result<Self> {
        ...
        // Synchronous validation - fast, CPU-bound
        if key.is_empty() {
            return Err(ValidationError::Empty { field: "api_key" });
        }

        if key.contains('\0') {
            return Err(ValidationError::InvalidCharacters { field: "api_key" });
        }

        Ok(Self(SecretString::new(key)))
    }
}
```

### Future Async Pattern (if needed)

If I/O-based validation is required later:

```rust
pub async fn validate_async(&self) -> Result<()> {
    // Check API key validity with external service
    let response = reqwest::get(format!("https://api.example.com/validate?key={}", self.expose()))
        .await?;
...
When adding a new LLM provider, follow this checklist:

### File Structure
- [ ] `provider_name/mod.rs` - Main provider implementation
- [ ] `provider_name/models.rs` - Provider-specific request/response types
- [ ] `provider_name/error.rs` - Error handling and mapping

### Core Implementation
- [ ] Implement `Provider` trait with all methods
- [ ] `name()` - Return provider identifier
- [ ] `transform_request()` - Convert unified request to provider format
- [ ] `execute()` - Make HTTP request with proper error handling
- [ ] `transform_response()` - Convert provider response to unified format
- [ ] `execute_stream()` - Implement streaming if supported (optional)

### Configuration
- [ ] Configure HTTP client with connection pooling
  - `pool_max_idle_per_host(10)`
  - `pool_idle_timeout(Duration::from_secs(90))`
  - `http2_prior_knowledge()` for HTTP/2
- [ ] Add rate limiting support via `with_rate_limit()`
- [ ] Support custom base URL via constructor

### Provider-Specific Handling
- [ ] Handle provider-specific headers (e.g., `x-api-key`, `anthropic-version`)
- [ ] Extract and handle system messages if needed (Anthropic)
- [ ] Map provider error codes to `ProviderError` enum
- [ ] Extract retry-after headers from 429 responses

### Observability
- [ ] Add metrics to `execute()` method:
  - Start timer at beginning
  - Record success with token usage
  - Record errors with error type
  - Record timeouts
- [ ] Add tracing for debugging:
  - Log request failures with status
  - Log error details
...
- [ ] Unit tests for transformation logic:
  ...
  - Error mapping
  - System message extraction (if applicable)
- [ ] Integration tests (ignored, require API key):
...
```rust
//! ProviderName API provider implementation.
//!
//! Supports:
//! - Model list
//! - Streaming responses
...
pub use models::*;
pub use error::ProviderNameError;
...
#[derive(Clone)]
pub struct ProviderNameProvider {
    api_key: ApiKey,
    base_url: String,
    client: Client,
...
impl std::fmt::Debug for ProviderNameProvider {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ProviderNameProvider")
            .field("base_url", &self.base_url)
            .finish_non_exhaustive()
    }
}

impl ProviderNameProvider {
    pub const DEFAULT_BASE_URL: &'static str = "https://api.provider.com/v1";
...
#[async_trait]
impl Provider for ProviderNameProvider {
    fn name(&self) -> &str {
        "provider_name"
    }
    ...
    async fn execute(&self, req: ProviderRequest) -> Result<ProviderResponse> {
        ...
        // Record metrics on success/error/timeout
        todo!()
    }

    fn transform_response(&self, resp: ProviderResponse) -> Result<CompletionResponse> {
        // Parse and transform provider response
        todo!()
    }
}
```
