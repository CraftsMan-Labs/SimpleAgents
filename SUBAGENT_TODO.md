...
| C1, C2, C3, C5, C6 | Main-Agent | `examples/workflow_email/**`, `bindings/go/examples/workflow_chat_history/**`, `docs/EXAMPLES.md`, `Makefile` | Cross-language examples need practical parity and low-friction run flows | Node/Go/Python chat-history examples + docs include parity flags and consistent make-based run commands across Node and Bun runtimes | completed | Added `run-node-chat-history` with `JS_RUNTIME=node|bun` plus shared-YAML docs |
