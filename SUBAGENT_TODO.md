...
| D3 | Main-Agent | `bindings/go/simpleagents_live_test.go`, `crates/simple-agents-napi/test/live.test.js` | Prevent future regressions in workflow streaming semantics by verifying against real provider responses | Live tests assert explicit workflow stream event types and reject legacy mixed raw event type | completed | Parent: `TODO.md` D3 |
| X1 | Main-Agent | `TODO.md`, `SUBAGENT_TODO.md`, `docs/**` | Cross-SDK work needs one non-ambiguous contract and execution map before code changes | Locked trace options contract and implementation plan aligned to binding surfaces | completed | Parent: `TODO.md` X1 |
| X2 | SA-Node-Email-Options | `crates/simple-agents-napi/src/lib.rs`, `crates/simple-agents-napi/index.d.ts`, `crates/simple-agents-napi/test/**` | Node email helper methods should match generic workflow options capability | Email helper methods accept and propagate `workflowOptions`; tests cover forwarding | completed | Parent: `TODO.md` X2 |
| X3 | SA-Go-Typed-Options | `bindings/go/simpleagents.go`, `bindings/go/*test.go`, `bindings/go/examples/**` | Go users need typed option shapes and helper methods for safer usage | Typed run option structs + convenience wrappers with backward-compatible map path | completed | Parent: `TODO.md` X3 |
| X4 | SA-Python-Stub-Parity | `crates/simple-agents-py/simple_agents_py.pyi`, `crates/simple-agents-py/tests/**` | Python stubs should match actual runtime signatures for editor/type-check parity | Stub signatures include workflow options where runtime supports them; tests guard API | completed | Parent: `TODO.md` X4 |
| X5, X6 | Main-Agent | Integration tests + docs sync + verification report | Ensure all subagent changes integrate and remain release-safe | Passing binding tests and updated docs/plan references in TODO artifacts | completed | Parent: `TODO.md` X5, X6 |
| E1, E2, E3, E4 | Main-Agent | `examples/workflow_email/python/**`, `examples/workflow_email/node/**`, `bindings/go/examples/workflow_email_all/**`, `examples/workflow_email/*README.md` | Users requested multi-language examples that cover all workflow YAML files | New Python/Node/Go runners execute all YAML files with shared input and docs include run commands | completed | Parent: `TODO.md` E1-E4 |
