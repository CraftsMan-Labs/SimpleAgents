...
| QR6 | SA-Workflow-Modularization | `crates/simple-agents-workflow/src/yaml_runner.rs`, `crates/simple-agents-workflow/src/runtime.rs` + extracted modules | God modules reduce maintainability and testability | Behavior-preserving extraction into focused modules with unchanged public outputs | in_progress | Added focused modules `yaml_runner/runner.rs`, `yaml_runner/api.rs`, `yaml_runner/context.rs`, `yaml_runner/llm_tools.rs`, `yaml_runner/globals.rs`, `yaml_runner/client_executor.rs`, `yaml_runner/validation.rs`, and `runtime/scope.rs`; deeper decomposition of execution pipeline remains |
