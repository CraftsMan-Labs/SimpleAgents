...
if [ -f examples/Cargo.toml ]; then
    ...
    sed -i.bak -E "s/(simple-agents-[a-z-]+ = \\{[^\\}]*version = \")[^\"]+/\1$WORKSPACE_VERSION/" examples/Cargo.toml
    sed -i.bak -E "s/(simple-agent-type = \\{[^\\}]*version = \")[^\"]+/\1$WORKSPACE_VERSION/" examples/Cargo.toml
...
for toml in crates/*/Cargo.toml; do
    ...
    if grep -q -E 'simple-agents-|simple-agent-type' "$toml"; then
        ...
        sed -i.bak -E "s/(simple-agent-type = \\{[^\\}]*version = \")[^\"]+/\1$WORKSPACE_VERSION/" "$toml"
        rm -f "${toml}.bak"
    fi
done
...
echo ""
echo "Changed files:"
git diff --name-only | grep -E '(\.toml|Cargo\.lock)$' || echo "  (no changes)"
