...
This document records the current, repository-visible performance picture for workflow execution across Rust, Python, and Node.js/TypeScript.
...
Peak memory consumption is not currently emitted as a first-class metric by the workflow runtime output payloads in any binding (Rust/Python/Node).
...
| Metric concept | Rust | Python | Node.js/TS |
| --- | --- | --- | --- |
| Total runtime | Criterion benchmark timing | `total_elapsed_ms` | `total_elapsed_ms` |
| Per-step runtime | Benchmark-specific (function timing) | `step_timings[].elapsed_ms` (return payload), `step_details[].elapsed_ms` (nerdstats) | `step_timings[].elapsed_ms` |
| Tokens/sec | Not in Criterion bench output | `tokens_per_second` | Via Rust workflow output when included |
| Peak memory | Not exposed | Not exposed | Not exposed |
...
This will produce comparable wall-clock + peak-memory baselines for Rust, Python, and Node.js.
